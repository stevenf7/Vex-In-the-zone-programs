...
#pragma config(Motor,  port1,           MobileGoalL,   tmotorVex393_HBridge, openLoop)
...
//	startTask (PID_ClawLift);
	...
	while (true){
		...
		if ((vexRT[Ch3Xmtr2]>0)&&(SensorValue[ArmPotL]<2225)){
			motor [TR]= motor [TL] = motor[BL] = motor[BR] = vexRT[Ch3Xmtr2];
		}
		else if (vexRT[Ch3Xmtr2]<0){
			motor [TR]= motor [TL]= motor[BL] = motor[BR] = vexRT[Ch3Xmtr2];
		}
		else{
			motor [TR]= motor [TL]= motor[BL] = motor[BR] = 0;
		}
